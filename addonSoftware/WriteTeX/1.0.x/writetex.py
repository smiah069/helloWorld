    ...
    def effect(self):
        ...
        if action == "viewold":
            ...
            if self.options.latexcmd.lower() == "xelatex":
                ...
                cmd = " CMD executed: {}".format('/Library/TeX/texbin/pdflatex "-output-directory=%s" -interaction=nonstopmode -halt-on-error "%s" > "%s"'
                                                 % (tmp_dir, tex_file, out_file))
                subprocess.call('/Library/TeX/texbin/pdflatex "-output-directory=%s" -interaction=nonstopmode -halt-on-error "%s" > "%s"'
                                % (tmp_dir, tex_file, out_file), shell=True)
            else:
            ...
            if not os.path.exists(pdf_file):
                ...
                if self.options.pdftosvg == '1':
                    subprocess.call('/usr/local/Cellar/pdf2svg/0.2.3_5/bin/pdf2svg %s %s' %
